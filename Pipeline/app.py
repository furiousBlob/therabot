...
ollama_client = OpenAI(base_url="http://127.0.0.1:11434/v1", api_key="EMPTY")
...
sos_contact_email_path = "onboarding-details/sos-contact-email.txt"
...
def send_email_internal(to_addr: str, subject: str, body: str) -> str:
    ...
    username = "test@gmail.com" # change this for your own
    password = f"{passkey}"
...
def send_email(body_content):
    ...
    subject = (
        f"""Summary of conversation with {user_full_name} on {formatted_date_time}"""
    )

    body = invoke_llm(
        f"""
    Please write an email body following these instructions:
    1. This email is a summary of a conversation between a user and a mental health chatbot named TheraBot.
    ...
    5. End the email with "From, TheraBot."
    ...
    for filename in os.listdir(DATA_PATH):
        ...
        if filename.endswith(".txt"):
            with open(filepath, "r", encoding="utf-8") as file:
                ...
                documents.append(
                    Document(page_content=file.read(), metadata={"source": filename})
                )
        else:
...
def generate_embeddings(text_chunks):
    ...
    for chunk in text_chunks:
        ...
        if "embedding" not in response:
            raise ValueError(
                f"Embedding generation failed for chunk: {chunk.page_content}"
            )
...
def perform_RAG(prompt):
    ...
    try:
        ...
        print("exisiting collection is", collection)
    except Exception:
    ...
    results = collection.query(
        query_embeddings=[prompt_embedding["embedding"]], n_results=5
    )
...
def emergency_calling():
    ...
    user_full_name = get_user_full_name()
    sos_contact_email = get_sos_contact_email() 
    user_contact_number = get_user_contact_number()  
    subject = f"Emergency Alert for {user_full_name}"
    
    # Email body
    body = f"""
    Hi,

    I am TheraBot, a mental health chatbot. {user_full_name} has chosen you as their SOS contact and might be in a possible emergency.

    I have emailed you the conversation details with me. Please contact them urgently to ensure their safety.

    User's Contact Number: {user_contact_number}

    Regards,
    TheraBot
    """

    # Call your send_email_internal function
    try:
        send_email_internal(
            to_addr=sos_contact_email,
            subject=subject,
            body=body
        )

        print(f"Email sent successfully to SOS contact: {sos_contact_email}")
    except Exception as e:
        print(f"Failed to send email to SOS contact: {e}")
    return
...
#         to=get_sos_contact_email(),
#         from_=twilio_number,
#         body=f"Hi, I am TheraBot. {user_full_name} has chosen you as their SOS contact and might be in an emergency. Please check your email for details and contact them urgently.",
#     )
...
#     #     to=get_sos_contact_email(),
#     #     from_=twilio_number,
#     #     twiml=f"""<Response><Say>Hi, I am TheraBot, {user_full_name} who has chosen you as your SOS contact is in a possible
#     #       emergency and needs your help! I have emailed you her conversation with me. Pleaes contact her urgently!!
#     #       Hi, I am TheraBot, {user_full_name} who has chosen you as your SOS contact is in a possible
#     #       emergency and needs your help! I have emailed you her conversation with me. Pleaes contact her urgently!!
#     #       </Say></Response>""",
#     # )
...
def save_onboarding_info(
    ...
    with open(sos_contact_email_path, "w") as file:
...
def get_sos_contact_email():
    with open(sos_contact_email_path, "r") as file:
        return file.read()
...
def check_emergency(prompt):
    ...
    is_emergency = any(
        re.search(rf"\b{keyword}\b", normalized_prompt)
...
with st.sidebar:
    ...
    with col1:
        st.button(
            "🏠",
            key="home_button",
            on_click=lambda: setattr(st.session_state, "active_tab", "Home"),
        )
    with col2:
        st.button(
            "Home",
    ...
    with col1:
        st.button(
            "📝",
            key="onboarding_button",
            on_click=lambda: setattr(st.session_state, "active_tab", "Onboarding"),
        )
    with col2:
        st.button(
            "Onboarding",
            key="onboarding_label",
            on_click=lambda: setattr(st.session_state, "active_tab", "Onboarding"),
        )
    ...
    with col1:
        st.button(
            "💬",
            key="chat_button",
            on_click=lambda: setattr(st.session_state, "active_tab", "Chat"),
        )
    with col2:
        st.button(
            "Chat",
    ...
    with col1:
        st.button(
            "🚪",
            key="endchat_button",
            on_click=lambda: setattr(st.session_state, "active_tab", "EndChat"),
        )
    with col2:
        st.button(
            "End Chat",
...
if st.session_state.active_tab == "Chat":
    ...
    new_LTM = invoke_llm(
        f"""summarize in around 20 lines without missing any details: {st.session_state.all_stm_summary + current_LTM}"""
    )
    ...
    st.markdown(
        """
        Your buddy - TheraBot is here to support your mental health journey. 
    ...
    st.markdown(
        ...
        - **Onboarding**: Set your preferences and get acquainted with the features of TheraBot.
        - **Chat**: Start a supportive chat with TheraBot.
        - **End Chat**: End the session when you're ready and inform your doctor!
        - **Delete Chat History**: Wipe off the old slate and start a new conversation!
    """
    )
    ...
    if st.session_state.page == "form":
        ...
        step1_complete = st.checkbox("Step 1: Personal Details")
        if step1_complete:
            user_full_name = st.text_input("Full Name")
            user_contact_number = st.text_input("Your Contact Number")

        progress_bar.progress(33 if step1_complete else 0)

        step2_complete = st.checkbox("Step 2: Doctor's Details")
        if step2_complete:
            doctor_name = st.text_input("Doctor's Name")
            doctor_email = st.text_input("Doctor's Email")

        progress_bar.progress(66 if step2_complete else (33 if step1_complete else 0))

        step3_complete = st.checkbox("Step 3: SOS Details")
        if step3_complete:
            sos_contact_name = st.text_input("SOS Contact Name")
            sos_contact_email = st.text_input("SOS Email")

        progress_bar.progress(
            100
            if step3_complete
            else (66 if step2_complete else (33 if step1_complete else 0))
        )

        if step1_complete and step2_complete and step3_complete:
            if st.button("Submit"):
